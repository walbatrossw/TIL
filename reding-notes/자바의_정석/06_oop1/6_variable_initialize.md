...
### 1) 명시적 초기화?
변수 선언과 동시에 초기화하는 것을 명시적 초기화(explicit initialization) 변수의 초기화라고 한다.
...
### 1) 초기화 블럭?
위에서 언급한 것과 같이 초기화 블럭에는 **클래스 초기화 블럭** 과 **인스턴스 초기화 블럭** 두가지 종류가 있다.

- 클래스 초기화 블럭
  - 클래스를 초기화하는데 사용
  - 클래스가 처음 메모리에 로딩될 때 한번만 수행
- 인스턴스 초기화 블럭
  - 인스턴스 변수를 초기화하는데 사용,
...
인스턴스 변수의 초기화는 주로 생성자를 사용하고, 인스턴스 초기화 블럭은 모든 생성자에서 공토으로 수행되어야 하는 코드를 넣는데 사용한다.
```java
Car() {
  count++;            // 코드 중복
...
### 3) 초기화 블럭 예제 2 : `static` 초기화 블럭(클래스 초기화 블럭)
```java
