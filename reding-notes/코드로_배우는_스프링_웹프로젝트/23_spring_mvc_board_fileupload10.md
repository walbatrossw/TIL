...
```
AJAX를 통해 파일 업로드 처리가 완료되면 업로드한 파일을 바로 화면에 출력하기 위해 파일 출력 메서드를 위와 같이 작성해준다.

요청을 받은 **파일 출력 메서드는 파일 데이터와 HttpHeader를 리턴** 해주게 되는데 파일데이터와 HttpHeaders를 처리하기 위한 과정은 아래와 같다.
1. 원하는 파일을 파라미터(`fileName`)로 받는다.
2. `httpHeaders`를 얻기 위해 `UploadFileUtils.getHttpHeaders(fileName)`메서드를 호출한다.
3. `getHttpHeaders(fileName)`메서드는 파일타입을 체크하여 설정에 맞는 MINE타입을 지정해 리턴해준다.
4. 원하는 파일을 `InputStream`객체를 통해 읽어온다.
5. 최종적으로 읽어온 파일 데이터와 HttpHeader를 호출한 화면으로 리턴해준다.
