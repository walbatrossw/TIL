...
일반적으로 검색기능의 구현은 **검색조건** 과 **검색키워드** 로 이루어진다. 검색 조건의 경우 select박스를 통해 사용자가 조건을 선택할 수 있도록 하고, 게시글의 제목, 내용, 작성자를 기준으로 아래와 같은 6가지의 경우의 수에 따라 검색결과를 반환하도록 구현한다.
...
## 3. 검색처리와 검색처리 정보를 유지하기 위한 JSP수정
